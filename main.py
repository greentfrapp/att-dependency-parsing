...
def main(unused_args):
	if FLAGS.train:
		...
		sess = tf.Session()
		model = AttentionModel2(sess, len(task.dict), max_len=task.bucket_lengths[FLAGS.bucket_id])
	...
	if FLAGS.test:
		...
		# max_len = 140
		sess = tf.Session()
		model = AttentionModel2(sess, len(task.dict), max_len=max_len)

		model.load(FLAGS.savepath)

		# x_tokens, x_pos, x_pos_2, y_in_parents, y_in_children, y_out_parents, y_out_children = dev_task.next_batch(64, FLAGS.bucket_id)
		x_tokens, x_pos, x_pos_2, _, y_out = task.next_batch(64, FLAGS.bucket_id)
		...
		for n_sample in np.arange(1):
			...
			while True:
				...
				seq.append(np.argmax(new))

				if np.argmax(new) == 0:
					break

			print(np.array(seq[1:]))
			print(np.argmax(y_out[n_sample], axis=1))
